...
static KYTY_SYSV_ABI uint64_t ResolveImportStubWithId(uint64_t record_id) {
	...
	if (record_id < g_stubbed_imports.size()) {
		...
		if (!nid.empty() &&
			...
			LOGF("Late-resolved import [thread=%d]: %s -> %s [0x%016" PRIx64 "]\n", tid, record.name.c_str(),
	...
	if (log_index < 1024) {
		if (record_id < g_stubbed_imports.size()) {
			...
			printf("Unresolved import stub called [thread=%d]: %s\n", tid, record.name.c_str());
			LOGF("Unresolved import stub called [%u] [thread=%d]: patch_vaddr=0x%016" PRIx64
			     " jmprela_index=%" PRIu32 " symbol=%s type=%s bind=%s program=%s\n",
			     log_index, tid, record.patch_vaddr, record.index, record.name.c_str(),
			     Common::EnumName(record.type).c_str(), Common::EnumName(record.bind).c_str(),
			     record.program.c_str());
		} else {
			printf("Unresolved import stub called [thread=%d]: <bad-record>\n", tid);
			LOGF("Unresolved import stub called [%u] [thread=%d]: record_id=%" PRIu64 " symbol=<bad-record>\n",
			     log_index, tid, record_id);
		}
	}
	return 0;
}
