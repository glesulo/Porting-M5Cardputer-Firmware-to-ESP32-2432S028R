...
int KYTY_SYSV_ABI PlayGoGetProgress(int handle, const uint16_t* chunk_ids,
	...
	constexpr uint64_t DEFAULT_CHUNK_SIZE_BYTES = 1024ull * 1024ull * 1024ull;
	const uint64_t total_bytes = static_cast<uint64_t>(number_of_entries) * DEFAULT_CHUNK_SIZE_BYTES;

	out_progress->total_size    = total_bytes;
	out_progress->progress_size = total_bytes;
